...
- backend custom fields 已补
- 分类已可创建到 backend
- 单商品价格、规格、主图、C 端图、分类归属已联调通过
- 商品相册同步链已补进发布 payload
...
原因不是 1 和 2 已“绝对完成”，而是：

- 1 和 2 已经足够支撑继续往前推进
- 继续把主要精力放在抓取和 backend 细节上，收益会递减
- 如果不先把分类页、商品页、多单位、B/C 图和客群过滤规则定下来，后续正式发布和前端接入仍会返工

因此当前建议调整为：

1. 主线进入“三、小程序 UI 必补能力”
2. 抓取链与 backend 发布链转为穿插收尾
3. UI contract 定稿后，再回补正式发布状态机和运营闭环
...
下一步最合理的是直接进入“三、小程序 UI 必补能力”的详细规划与 contract 定稿。

建议先看：

- [miniapp-ui-plan.md](./miniapp-ui-plan.md)

然后按这份文档的批次推进：

1. 分类页与分类切换模型
2. 商品页与多单位交互
3. B/C 图与 `targetAudience`
4. 购物车与结算展示 contract

抓取链与 backend 发布链则保留为收尾批次并行推进。
