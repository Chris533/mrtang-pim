...
`源站抓取入库` 是 `mrtang-pim` 后台里承接“目标站分类、商品规格、图片资产”抓取入库的模块。
...
- 抓取商品、规格、多单位价格到 `source_products`
...
2. 全量分类抓取入库可启动，并出现进度和阶段日志
3. 全量商品规格抓取入库可启动，并出现进度和阶段日志
4. 全量图片抓取入库可启动，并出现进度和阶段日志
5. 运行详情页可打开
6. 商品变更后会回到 `imported`
7. 图片变更后会回到 `pending`
8. `go test ./...` 通过
